...
class HX711:
	...
	def moving_average(self, val):
		self.movavg[self.movavghead] = val
		self.movavghead += 1
		if self.movavghead == self.movavgSIZE:
			self.movavghead = 0
		self.movavgsize += 1
		if self.movavgsize > self.movavgSIZE:
			self.movavgsize = self.movavgSIZE

		value = 0
		for i in range(self.movavgsize):
			value += self.movavg[i]
		value = value / self.movavgsize
	...
	def read_average(self, times=3):
		for i in range(times):
			...
			retval = self.moving_average(value)
